...
class Parser:
    ...
    def clear_items(self, item=None):
        to_clear = {
            "table": "",
            "column": "",
            "type": "",
    ...
    def parse_items(self):
        for item in self.generated:
            ...
            if self.items["column"] and self.items["value"]:
                self.items["values"].append({self.items["column"]: self.items["value"]})
    ...
    def parse_from_dict(self, _json):
        text = ""

        for item in _json:
            text += f"DB:{item}\n"

            for item2 in _json[item]:
                text += f"    TABLE:{item2}\n"
                columns = []

                for item3 in _json[item][item2]:
                    column = list(item3.keys())[0]
                
                    if not column in columns:
                        text += f"        COLUMN:{column}\n"
                        columns.append(column)

                    text += "".join([f"            TYPE:{type(item3[x]).__name__}\n            VALUE:{self.types2[type(item3[x]).__name__](item3[x])}\n" for x in item3])
